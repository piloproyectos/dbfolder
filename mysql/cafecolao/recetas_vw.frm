TYPE=VIEW
query=select `p`.`id` AS `id_producto`,`p`.`nombre` AS `nombre_producto`,`p`.`fk_unidad` AS `unidad_id_producto`,`u1`.`descripcion` AS `unidad_descripcion_producto`,`tp`.`id` AS `tipo_id_producto`,`tp`.`descripcion` AS `tipo_descripcion_producto`,`i`.`id` AS `id_ingrediente`,`i`.`nombre` AS `nombre_ingrediente`,`i`.`fk_unidad` AS `unidad_id_ingrediente`,`u2`.`descripcion` AS `unidad_descripcion_ingrediente`,`r`.`cantidad` AS `cantidad` from (((((`cafecolao`.`productos` `p` join `cafecolao`.`recetas` `r` on((`p`.`id` = `r`.`fk_producto`))) join `cafecolao`.`productos` `i` on((`i`.`id` = `r`.`fk_ingrediente`))) join `cafecolao`.`unidades` `u1` on((`p`.`fk_unidad` = `u1`.`id`))) join `cafecolao`.`unidades` `u2` on((`i`.`fk_unidad` = `u2`.`id`))) join `cafecolao`.`tipo_productos` `tp` on((`p`.`fk_tipo` = `tp`.`id`))) where ((`p`.`fk_tipo` in (2,4,6)) and (`i`.`fk_tipo` = 5))
md5=0d4452835ee21dcda805c592c147b5cc
updatable=1
algorithm=0
definer_user=root
