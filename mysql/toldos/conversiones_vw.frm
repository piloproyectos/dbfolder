TYPE=VIEW
query=(select `p`.`id` AS `id_producto`,`p`.`nombre` AS `nombre_producto`,`u1`.`id` AS `unidad_id`,`u1`.`descripcion` AS `unidad_descripcion`,`con`.`cantidad` AS `equi_cantidad`,`u2`.`id` AS `equi_unidad_id`,`u2`.`descripcion` AS `equi_unidad_descripcion` from (((`toldos`.`productos` `p` join `toldos`.`conversiones` `con` on((`p`.`id` = `con`.`fk_producto`))) join `toldos`.`unidades` `u1` on((`p`.`fk_unidad` = `u1`.`id`))) join `toldos`.`unidades` `u2` on((`u2`.`id` = `con`.`fk_unidad`))))
md5=d82bfba5f9f359cdc31159657f9f3770
updatable=1
algorithm=0
definer_user=root
