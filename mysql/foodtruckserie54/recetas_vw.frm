TYPE=VIEW
query=select `p`.`id` AS `id_producto`,`p`.`nombre` AS `nombre_producto`,`p`.`fk_unidad` AS `unidad_id_producto`,`u1`.`descripcion` AS `unidad_descripcion_producto`,`tp`.`id` AS `tipo_id_producto`,`tp`.`descripcion` AS `tipo_descripcion_producto`,`i`.`id` AS `id_ingrediente`,`i`.`nombre` AS `nombre_ingrediente`,`i`.`fk_unidad` AS `unidad_id_ingrediente`,`u2`.`descripcion` AS `unidad_descripcion_ingrediente`,`r`.`cantidad` AS `cantidad` from (((((`foodtruckserie54`.`productos` `p` join `foodtruckserie54`.`recetas` `r` on((`p`.`id` = `r`.`fk_producto`))) join `foodtruckserie54`.`productos` `i` on((`i`.`id` = `r`.`fk_ingrediente`))) join `foodtruckserie54`.`unidades` `u1` on((`p`.`fk_unidad` = `u1`.`id`))) join `foodtruckserie54`.`unidades` `u2` on((`i`.`fk_unidad` = `u2`.`id`))) join `foodtruckserie54`.`tipo_productos` `tp` on((`p`.`fk_tipo` = `tp`.`id`))) where ((`p`.`fk_tipo` in (2,4,6)) and (`i`.`fk_tipo` = 5))
md5=5712898da448f7b12fc76e5531d6da79
updatable=1
algorithm=0
definer_user=root
