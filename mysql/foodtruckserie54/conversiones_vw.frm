TYPE=VIEW
query=(select `p`.`id` AS `id_producto`,`p`.`nombre` AS `nombre_producto`,`u1`.`id` AS `unidad_id`,`u1`.`descripcion` AS `unidad_descripcion`,`con`.`cantidad` AS `equi_cantidad`,`u2`.`id` AS `equi_unidad_id`,`u2`.`descripcion` AS `equi_unidad_descripcion` from (((`foodtruckserie54`.`productos` `p` join `foodtruckserie54`.`conversiones` `con` on((`p`.`id` = `con`.`fk_producto`))) join `foodtruckserie54`.`unidades` `u1` on((`p`.`fk_unidad` = `u1`.`id`))) join `foodtruckserie54`.`unidades` `u2` on((`u2`.`id` = `con`.`fk_unidad`))))
md5=373064682129268ba6b4106b670c186a
updatable=1
algorithm=0
definer_user=root
