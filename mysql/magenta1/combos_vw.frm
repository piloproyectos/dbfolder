TYPE=VIEW
query=select `pc`.`id` AS `id_combo`,`pc`.`nombre` AS `nombre_combo`,`pc`.`fk_unidad` AS `unidad_id_combo`,`u2`.`descripcion` AS `unidad_descripcion_combo`,`tp`.`id` AS `tipo_id_combo`,`tp`.`descripcion` AS `tipo_descripcion_combo`,`p`.`id` AS `id_producto`,`p`.`nombre` AS `nombre_producto`,`p`.`fk_unidad` AS `unidad_id_producto`,`u1`.`descripcion` AS `unidad_descripcion_producto`,`c`.`cantidad` AS `cantidad` from (((((`magenta1`.`productos` `p` join `magenta1`.`combos` `c` on((`p`.`id` = `c`.`fk_producto`))) join `magenta1`.`productos` `pc` on((`pc`.`id` = `c`.`fk_combo`))) join `magenta1`.`unidades` `u1` on((`p`.`fk_unidad` = `u1`.`id`))) join `magenta1`.`unidades` `u2` on((`pc`.`fk_unidad` = `u2`.`id`))) join `magenta1`.`tipo_productos` `tp` on((`pc`.`fk_tipo` = `tp`.`id`))) where ((`p`.`fk_tipo` in (1,2,6)) and (`pc`.`fk_tipo` = 3))
md5=d758021c965d5db722fe94b88871c8c7
updatable=1
algorithm=0
definer_user=root
