TYPE=VIEW
query=select `p`.`id` AS `id`,`p`.`nombre` AS `nombre`,`p`.`tiempo_preparacion` AS `tiempo_preparacion`,`p`.`valor` AS `valor`,`p`.`descripcion` AS `descripcion`,`p`.`fk_estado` AS `fk_estado`,`p`.`visible` AS `visible`,`p`.`tamano_def` AS `tamano_def`,`p`.`fk_categoria` AS `fk_categoria`,`p`.`fk_tipo` AS `fk_tipo`,`p`.`fk_unidad` AS `fk_unidad`,`p`.`fk_tamano_of` AS `fk_tamano_of`,`p`.`anexos` AS `anexos`,`p`.`fk_cocina` AS `fk_cocina`,`p`.`codigo` AS `codigo`,`p`.`referencia` AS `referencia`,`p`.`imp_consumo` AS `imp_consumo`,`p`.`iva` AS `iva`,`p`.`stock_min` AS `stock_min` from `magenta2`.`productos` `p` where ((`p`.`fk_tipo` = 6) and (`p`.`fk_tamano_of` is not null))
md5=d939dc0b1e4ddd2b3b00c21bebe31eda
updatable=1
algorithm=0
definer_user=root
